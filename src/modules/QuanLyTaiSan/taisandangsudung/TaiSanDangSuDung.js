...
const TaiSanDangSuDung = (state) => {
  ...
  const requestDatas = () => {
    ...
    if (datas && datas.length > 0) {
      let url = `${endPoint.getTaiSanDangSuDung}?`;
  
      const textState = state.searchText;
      const textFilter = find(textState, itemSelected => itemSelected.screen === screens.quan_ly_tai_san && itemSelected.tab === tabs.tai_san_dang_su_dung)
        && find(textState, itemSelected => itemSelected.screen === screens.quan_ly_tai_san && itemSelected.tab === tabs.tai_san_dang_su_dung).data;
      if (textFilter) {
...
export default connect(mapStateToProps)(TaiSanDangSuDung);