...
const TaiSanThanhLy = (state) => {
  ...
  const requestDatas = () => {
    ...
    if (datas && datas.length > 0) {
      let url = `${endPoint.getTaiSanThanhLy}?`;
  
      const textState = state.searchText;
      const textFilter = find(textState, itemSelected => itemSelected.screen === screens.quan_ly_tai_san && itemSelected.tab === tabs.tai_san_thanh_ly)
        && find(textState, itemSelected => itemSelected.screen === screens.quan_ly_tai_san && itemSelected.tab === tabs.tai_san_thanh_ly).data;
      if (textFilter) {
  ...
  const LoadScreenThemmoi = () => props.navigation.navigate(screens.khai_bao_tai_san, { screen: "tài sản thanh lý", onGoBack: () => refreshData() })
  return (
...
export default connect(mapStateToProps)(TaiSanThanhLy);