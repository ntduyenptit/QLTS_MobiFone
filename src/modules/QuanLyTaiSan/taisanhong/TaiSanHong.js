...
const TaiSanHong = (state) => {
  ...
  const requestDatas = () => {
    ...
    if (datas && datas.length > 0) {
      let url = `${endPoint.getTaiSanHong}?`;
  
      const textState = state.searchText;
      const textFilter = find(textState, itemSelected => itemSelected.screen === screens.quan_ly_tai_san && itemSelected.tab === tabs.tai_san_hong)
        && find(textState, itemSelected => itemSelected.screen === screens.quan_ly_tai_san && itemSelected.tab === tabs.tai_san_hong).data;
      if (textFilter) {
  ...
  const LoadScreenThemmoi = () => props.navigation.navigate(screens.khai_bao_tai_san, { screen: "tài sản hỏng", onGoBack: () => refreshData() })
  return (
...
export default connect(mapStateToProps)(TaiSanHong);